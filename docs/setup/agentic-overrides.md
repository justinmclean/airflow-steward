...
Framework skills consult **two** override directories and apply
**both** if present — this is layering, not first-match. Where the
two touch the same behaviour, the personal-local surface takes
precedence:
...
When both directories contain a file for the same skill, both are
applied: the committed, project-wide overrides form the base and the
personal-local overrides layer on top.  Where a personal-local
instruction conflicts with a committed one, the personal-local
instruction wins; a personal-local override may also explicitly
cancel a committed one.  Overrides that do not conflict simply
compose.  Neither file is required to exist; a skill that finds
neither proceeds with framework defaults.
...
4. Apply both sets of overrides, layered: the committed,
   project-wide overrides are the base and the personal-local
   overrides layer on top, so a personal-local instruction wins
   wherever it conflicts with a committed one.  Each
   `### Override N — ...` section modifies the skill's default
   behaviour for this run.  The agent interprets the instructions
