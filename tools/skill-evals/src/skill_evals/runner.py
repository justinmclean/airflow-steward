...
def batch_judge_assertions(
    ...
    try:
        ...
        return {k: (None, f"grader CLI timed out after {timeout}s") for k in specs}
    except OSError as exc:
        return {k: (None, f"grader CLI invocation failed ({exc})") for k in specs}
    if rc != 0:
        return {k: (None, f"grader CLI exited {rc} ({stderr.strip()[:200]})") for k in specs}
    verdict, err = extract_json_from_output(stdout)
    if err is not None or not isinstance(verdict, dict):
        return {k: (None, f"grader returned unusable output ({err or 'not a dict'})") for k in specs}
